...
/**
 * Managerrateperiod model.
 *
 * @since  2.0.1
 */
class ManagerrateperiodModel extends AdminModel
{
	/**
	 ...
	 * @since  2.0.1
	 */
	public $typeAlias = 'com_accommodation_manager.managerrateperiod';
	 ...
	 * @since   2.0.1
	 */
	public function getTable($type = 'Managerrateperiod', $prefix = 'Administrator', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true)
	{
		// Get the form.
		$form = $this->loadForm(
								'com_accommodation_manager.managerrateperiod', 
								'managerrateperiod',
								array(
									'control' => 'jform',
									'load_data' => $loadData 
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_accommodation_manager.edit.managerrateperiod.data', array());
	...
	/**
	 * Method to duplicate an Managerrateperiod
	 *
	...
	protected function prepareTable($table)
	{
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDatabase();
				$db->setQuery('SELECT MAX(ordering) FROM #__accommodation_manager_rate_periods');
				$max             = $db->loadResult();
