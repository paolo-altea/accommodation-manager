	...
	protected function addToolbar()
	{
		...
		if (!$checkedOut && ($canDo->get('core.edit') || $canDo->get('core.create')))
		{
			...
			$saveGroup->configure(
				...
				{
					// Save (apply)
					$childBar->apply('managerrateperiod.apply');

					// Save & Close
					$childBar->save('managerrateperiod.save');

					// Save & New
					if ($canDo->get('core.create'))
					{
						$childBar->save2new('managerrateperiod.save2new');
					}

					// Save as Copy
					if (!$isNew && $canDo->get('core.create'))
					{
						$childBar->save2copy('managerrateperiod.save2copy');
					}
				}
			);
		}
		...
		if ($isNew)
		{
			$toolbar->cancel('managerrateperiod.cancel', 'JTOOLBAR_CANCEL');
		}
		else
		{
			$toolbar->cancel('managerrateperiod.cancel', 'JTOOLBAR_CLOSE');
		}
	}
}
