...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar()
	{
		...
		ToolbarHelper::title(Text::_('COM_ACCOMMODATION_MANAGER_TITLE_MANAGERRATETYPOLOGIES'), "generic");

		$toolbar = Toolbar::getInstance('toolbar');

		// Check if the form exists before showing the add/edit buttons
		$formPath = JPATH_COMPONENT_ADMINISTRATOR . '/src/View/Managerratetypologies';

		if (file_exists($formPath))
		{
			if ($canDo->get('core.create'))
			{
				$toolbar->addNew('managerratetypology.add');
			}
		}
		...
		if ($canDo->get('core.edit.state')  || count($this->transitions))
		{
			...
			{
				$childBar->publish('managerratetypologies.publish')->listCheck(true);
				$childBar->unpublish('managerratetypologies.unpublish')->listCheck(true);
				$childBar->archive('managerratetypologies.archive')->listCheck(true);
			}
			elseif (isset($this->items[0]))
			{
				// If this component does not use state then show a direct delete button as we can not trash
				$toolbar->delete('managerratetypologies.delete')
			...
			$childBar->standardButton('duplicate')
				...
				->icon('fas fa-copy')
				->task('managerratetypologies.duplicate')
				->listCheck(true);

			if (isset($this->items[0]->checked_out))
			{
				$childBar->checkin('managerratetypologies.checkin')->listCheck(true);
			}

			if (isset($this->items[0]->state))
			{
				$childBar->trash('managerratetypologies.trash')->listCheck(true);
			}
		}
