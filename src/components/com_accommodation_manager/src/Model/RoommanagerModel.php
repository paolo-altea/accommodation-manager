 ...
 * @since  2.0.1
 */
class RoommanagerModel extends ItemModel
{
	public $_item;
	...
	protected function populateState()
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_accommodation_manager')) && (!$user->authorise('core.edit', 'com_accommodation_manager')))
		{
	...
	public function getItem($id = null)
	{
		if ($this->_item === null)
		{
			...
			if ($table && $table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new \Exception(Text::_('COM_ACCOMMODATION_MANAGER_ITEM_NOT_LOADED'), 403);
					}
				}
			...
			if (empty($this->_item))
			{
				throw new \Exception(Text::_('COM_ACCOMMODATION_MANAGER_ITEM_NOT_LOADED'), 404);
			}
		}
		...
		if (isset($this->_item->room_category) && $this->_item->room_category != '')
		{
			if (is_object($this->_item->room_category))
			{
				$this->_item->room_category = ArrayHelper::fromObject($this->_item->room_category);
			}

			$values = (is_array($this->_item->room_category)) ? $this->_item->room_category : explode(',',$this->_item->room_category);
			...
			$this->_item->room_category = !empty($textValue) ? implode(', ', $textValue) : $this->_item->room_category;
	...
	public function getItemIdByAlias($alias)
	{
		...
		{
			$aliasKey   = $this->getAliasFieldNameByView('roommanager');
		}
